...
const createSet = value => {
  if (_.isArray(value)) {
    return docClient.createSet(value);
  } else {
    return docClient.createSet([value]);
  }
};
...
const serialize = {
  ...
  binarySet: function (value) {
    ...
    return createSet(vals, 'B');
  }
};

function deserializeAttribute(value) {
...
function serializeAttribute(value, type, options) {
...
function serializeItem(schema, item, options) {
  options = options || {};
  ...
  const serialize = (item, datatypes) => {
    ...
    return _.reduce(item, (result, val, key) => {
      ...
      const attr = serializeAttribute(val, datatypes[key], options);
...
function buildKey(hashKey, rangeKey, schema) {
  const obj = {};
  ...
  return serializeItem(schema, obj);
}

function serializeItemForUpdate(schema, action, item) {
  ...
  return _.reduce(data, (result, value, key) => {
    if (_.isNull(value)) {
      ...
      result[key] = { Action: 'ADD', Value: serializeAttribute(value.$add, datatypes[key]) };
    } else if (_.isPlainObject(value) && value.$del) {
      result[key] = { Action: 'DELETE', Value: serializeAttribute(value.$del, datatypes[key]) };
    } else {
      result[key] = { Action: action, Value: serializeAttribute(value, datatypes[key]) };
    }
  ...
  const formatter = data => {
    ...
    return map(data, value => {
      ...
      if (_.isPlainObject(value)) {
        ...
        result = deserializeAttribute(value);
      }
...
module.exports = {
  ...
  serializeItemForUpdate: serializeItemForUpdate,
