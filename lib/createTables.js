...
function waitTillActive(options, model, callback) {
  let status = 'PENDING';
...
function createTable(model, globalOptions, options, callback) {
  ...
  model.describeTable((err, data) => {
    if (_.isNull(data) || _.isUndefined(data)) {
      ...
      return model.createTable(options, error => {
        ...
        waitTillActive(globalOptions, model, callback);
      });
    } else {
...
function createTables(models, config, callback) {
  async.eachSeries(_.keys(models), (key, callback) => createTable(models[key], config.$dynogels, config[key], callback), callback);
}
