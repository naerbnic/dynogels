...
function omitNulls(data) {
  return _.omitBy(data, value => _.isNull(value) ||
    _.isUndefined(value) ||
    (_.isArray(value) && _.isEmpty(value)) ||
    (_.isString(value) && _.isEmpty(value)));
}

function mergeResults(responses, tableName) {
  const result = {
    Items: [],
    ConsumedCapacity: {
      CapacityUnits: 0,
...
function streamRequest(self, runRequestFunc) {
...
function paginatedRequest(self, runRequestFunc, callback) {
  // if callback isn't passed switch to stream
  if (!callback) {
    return streamRequest(self, runRequestFunc);
  }
...
function omitPrimaryKeys(schema, params) {
...
utils.omitNulls = omitNulls;
utils.mergeResults = mergeResults;
utils.paginatedRequest = paginatedRequest;
utils.streamRequest = streamRequest;
utils.omitPrimaryKeys = omitPrimaryKeys;
utils.strToBin = strToBin;
