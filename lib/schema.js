...
const secondaryIndexSchema = Joi.object().keys({
...
const configSchema = Joi.object().keys({
  ...
  indexes: Joi.array().items(secondaryIndexSchema),
  schema: Joi.object(),
...
function invokeDefaultFunctions(data) {
  return _.mapValues(data, val => {
    if (_.isPlainObject(val)) {
      return invokeDefaultFunctions(val);
    } else {
      return val;
    }
  });
...
function Schema(config) {
  ...
  const self = this;
  Joi.validate(config, configSchema, { context: context }, (err, data) => {
    if (err) {
    ...
    self._modelDatatypes = parseDynamoTypes(self._modelSchema.describe());
  });
}
...
Schema.prototype.applyDefaults = function (data) {
  ...
  return invokeDefaultFunctions(result.value);
};
