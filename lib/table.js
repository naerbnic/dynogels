...
function attributeDefinition(schema, key) {
...
function keySchema(hashKey, rangeKey) {
  const result = [{
...
function callBeforeHooks(table, name, startFun, callback) {
  const listeners = table._before.listeners(name);

  return async.waterfall([startFun].concat(listeners), callback);
}

function addConditionExpression(params, expectedConditions) {
...
function createItem(table, item, options, callback) {
  const self = table;
  ...
  callBeforeHooks(self, 'create', start, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    if (options.expected) {
      addConditionExpression(params, options.expected);
    ...
    if (options.overwrite === false) {
      ...
      addConditionExpression(params, expected);
    }
...
function findMissingGlobalIndexes(table, data) {
...
function globalIndex(indexName, params) {
  const projection = params.projection || { ProjectionType: 'ALL' };

  return {
    IndexName: indexName,
    KeySchema: keySchema(params.hashKey, params.rangeKey),
...
function syncIndexes(table, callback) {
  ...
  table.describeTable((err, data) => {
    ...
    const missing = _.values(findMissingGlobalIndexes(table, data));
    ...
    async.mapLimit(missing, maxIndexCreationsAtaTime, (params, callback) => {
      ...
      if (!_.find(attributeDefinitions, { AttributeName: params.hashKey })) {
        attributeDefinitions.push(attributeDefinition(table.schema, params.hashKey));
      }

      if (params.rangeKey && !_.find(attributeDefinitions, { AttributeName: params.rangeKey })) {
        attributeDefinitions.push(attributeDefinition(table.schema, params.rangeKey));
      }
      ...
      const updateParams = {
        ...
        GlobalSecondaryIndexUpdates: [{ Create: globalIndex(params.name, params) }]
      };
...
function secondaryIndex(schema, params) {
  const projection = params.projection || { ProjectionType: 'ALL' };

  return {
    IndexName: params.name,
    KeySchema: keySchema(schema.hashKey, params.rangeKey),
    Projection: projection
  };
}

function updateTableCapacity(table, throughput, callback) {
  const params = {};
...
Table.prototype.create = function (item, options, callback) {
  ...
  if (_.isArray(item)) {
    async.map(item, (data, callback) => createItem(self, data, options, callback), callback);
  } else {
    return createItem(self, item, options, callback);
  }
};

function updateExpressions(schema, data, options) {
...
function validateItemFragment(item, schema) {
  const result = {};
  const error = {};
...
Table.prototype.update = function (item, options, callback) {
  ...
  const schemaValidation = validateItemFragment(item, self.schema);
  ...
  callBeforeHooks(self, 'update', start, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    let exp = null;
    try {
      exp = updateExpressions(self.schema, data, options);
    } catch (e) {
      return callback(e);
    }
...
function deserializeItems(table, callback) {
  return (err, data) => {
    if (err) {
      return callback(err);
    }
...
Table.prototype.runQuery = function (params, callback) {
  ...
  self.sendRequest('query', params, deserializeItems(self, callback));
};

Table.prototype.runScan = function (params, callback) {
  const self = this;

  self.sendRequest('scan', params, deserializeItems(self, callback));
};
...
Table.prototype.createTable = function (options, callback) {
  ...
  attributeDefinitions.push(attributeDefinition(self.schema, self.schema.hashKey));

  if (self.schema.rangeKey) {
    attributeDefinitions.push(attributeDefinition(self.schema, self.schema.rangeKey));
  }

  const localSecondaryIndexes = [];

  _.forEach(self.schema.secondaryIndexes, params => {
    attributeDefinitions.push(attributeDefinition(self.schema, params.rangeKey));
    localSecondaryIndexes.push(secondaryIndex(self.schema, params));
  });
  ...
  _.forEach(self.schema.globalIndexes, (params, indexName) => {
    if (!_.find(attributeDefinitions, { AttributeName: params.hashKey })) {
      attributeDefinitions.push(attributeDefinition(self.schema, params.hashKey));
    }
    ...
    globalSecondaryIndexes.push(globalIndex(indexName, params));
  });

  const newKeySchema = keySchema(self.schema.hashKey, self.schema.rangeKey);
...
Table.prototype.updateTable = function (throughput, callback) {
  ...
  async.parallel([
    async.apply(syncIndexes, self),
    async.apply(updateTableCapacity, self, throughput),
  ], callback);
};
